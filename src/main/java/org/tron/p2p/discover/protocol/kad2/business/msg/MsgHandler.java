...
public class MsgHandler {
  ...
  public void handleMsg(Message msg, InetSocketAddress address) {
    switch (msg.getType()) {
      ...
      default: {
        log.debug("Receive unknow message {} from {}", msg.getType(), address);
        break;
      }
    }
  }
  ...
  private boolean check(RequestMessage msg, InetSocketAddress address) {
    ...
    if (session == null || !session.getKey().equals(msg.getSession())) {
      log.debug("Receive FindNodes from {} without session {},{}.", address, msg.getSession(), session);
      handshakeService.handleUnknow(address, msg.getReqId());
      return false;
    }
